...
import { completeLeadCadence } from "@/lib/data/cadences";
...
export async function POST(request: Request, props: RouteContext) {
  ...
  try {
    const cadence = await completeLeadCadence(params.leadCadenceId, parsed.data);
    return NextResponse.json({ data: cadence, success: true });
  } catch (error) {
    return jsonError(
      error instanceof Error ? error.message : "Không thể hoàn thành quy trình.",
      500,
    );
  }
}
