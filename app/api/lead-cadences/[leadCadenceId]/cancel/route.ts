...
import { cancelLeadCadence } from "@/lib/data/cadences";
...
export async function POST(request: Request, props: RouteContext) {
  ...
  try {
    const cadence = await cancelLeadCadence(params.leadCadenceId, parsed.data);
    return NextResponse.json({ data: cadence, success: true });
  } catch (error) {
    return jsonError(
      error instanceof Error ? error.message : "Không thể hủy quy trình.",
      500,
    );
  }
}
