...
const DEFAULT_MAPS_ACCESS_MESSAGE =
  "Google Maps API key chưa được cấp quyền, sai hạn chế truy cập, hoặc chưa bật API/billing cần thiết.";
...
export function getMapProviderApiError(
  ...
  if (code === "MAP_PROVIDER_ACCESS_DENIED") {
    return {
  ...
  if (code === "MAP_PROVIDER_QUOTA_EXCEEDED") {
    return {
